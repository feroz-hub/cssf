...
/// <summary>
/// EF Core entity type configuration that maps the <see cref="ApiResourceClaims"/> entity
/// to the "HCL.CS.SF_ApiResourceClaims" table. Configures a unique composite index on
/// (ApiResourceId, Type) to ensure each claim type is unique per API resource.
/// </summary>
public class ApiResourceClaimsMap
{
    /// <summary>
    /// Initializes the API resource claims mapping.
    /// </summary>
    /// <param name="entityBuilder">The entity type builder for <see cref="ApiResourceClaims"/>.</param>
    ...
    private void ApiClaimsMapping(EntityTypeBuilder<ApiResourceClaims> entityBuilder)
    {
        ...
        {
            entityBuilder.ToTable("HCL.CS.SF_ApiResourceClaims");
